...
class _HomePageState extends State<HomePage> {
  ...
  Widget _buildPizza(Pizza pizza,
  ...
  List<Widget> _buildPizzaList() {
    ...
    for (int i = 0; i < _pizzas.length; i++) {
      Widget pizza;
      if (i == _currentIndex) {
        pizza = _buildPizza(_pizzas[i]);
      } else if (i == _currentIndex - 1) {
        //left
        pizza = _buildPizza(_pizzas[i], PizzaGeometry.left());
      } else if (i == _currentIndex + 1) {
        //right
        pizza = _buildPizza(_pizzas[i], PizzaGeometry.right());
      } else if (i < _currentIndex) {
        //prev
        pizza = _buildPizza(_pizzas[i], PizzaGeometry.prev());
      } else {
        //next
        pizza = _buildPizza(_pizzas[i], PizzaGeometry.next());
      }
      pizzaList.add(pizza);
    }
    return pizzaList;
